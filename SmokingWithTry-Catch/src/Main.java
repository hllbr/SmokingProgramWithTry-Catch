...
class SigaraKontrol extends Exception{

    @Override
    public void printStackTrace() {
        System.out.println("Sigara paketinde 3 dal kalamaz\n"
                + "Lütfen zehir Paketini tazele");
    }
    
}
class FiyatKontrol extends Exception{

    @Override
    public void printStackTrace() {
        System.out.println("Veresiyemiz yoktur.");
    }
...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        ...
        while (true) {  
            ...
            if(islem.equals("1")){
                ...
                if(toplamSonuc<0){
                 ...
                 System.out.println("********************************************");
                    ...
                    while(sigaraadet != 0){
                        sigara.poll();
                         System.out.println(sigaraadet+"  .(ıncı/inci) sigara içiliyor");
                             Thread.sleep(1000);
                                sigaraadet--;
                                if(sigaraadet <=3){
                                    throw new SigaraKontrol();
                                }
                    }
            ...
            }catch(SigaraKontrol e){//özsel hata
                e.printStackTrace();
    }catch(FiyatKontrol e){//özel hata
        e.printStackTrace();
    }catch(ArithmeticException ex){//Genel hata
             System.out.println("Matematiksel(Mantıksal) bie hata gerçekleşti...");
    }
